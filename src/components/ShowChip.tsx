...
export const ShowChip = forwardRef<HTMLDivElement, Props>(function ShowChip(
  { show, matched, isCurrentMatch, showVenueBadge, compact, dense },
  ref,
) {
  ...
  return (
    <div
      ...
      className={[
        ...
        compact ? (dense ? 'p-1 flex-1 min-h-0' : 'p-1.5 flex-1 min-h-0') : 'p-2',
      ...
      tabIndex={0}
    >
      <div className={`flex items-start min-w-0 ${dense ? 'gap-1' : 'gap-1.5'}`}>
        <button
          onClick={(e) => {
            e.stopPropagation();
            toggle(show.id);
          ...
          aria-pressed={saved}
          className={[
            'shrink-0 rounded-full border flex items-center justify-center text-[10px] font-bold transition-colors',
        ...
        <div className="min-w-0 flex-1">
          {dense ? (
            <div className="flex items-baseline gap-1.5 min-w-0">
              <span className="text-accent font-mono shrink-0 text-[10px]">{show.timeLabel}</span>
              {headliner && <span className="text-headliner text-[10px] shrink-0">★</span>}
              <span className="text-[10px] text-zinc-100 truncate">{show.title}</span>
            </div>
          ) : (
            <>
